...
import type { Quest, QuestDifficulty, QuestType } from "../types";
	...
	const filteredQuests = useMemo(() => {
		return quests.filter((quest) => {
			...
			const matchesLocation =
				filters.location === "all" || quest.location === filters.location;
			const matchesReward =
				filters.reward === "all" || quest.reward.type === filters.reward;
				...
				<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
					...
					<div>
						...
						<select
							...
							onChange={(e) =>
								setFilters((prev) => ({ ...prev, reward: e.target.value }))
							}
							className="block w-full pl-3 pr-10 py-2 text-base border border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 rounded-md"
						>
					...
					<div>
						...
						<select
							...
							onChange={(e) =>
								setFilters((prev) => ({
									...prev,
									difficulty: e.target.value as QuestDifficulty | "all",
								}))
							}
							className="block w-full pl-3 pr-10 py-2 text-base border border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 rounded-md"
						>
							...
							<div className="p-6">
								...
								<div className="mt-4">
									<div className="flex justify-between text-sm text-gray-500">
										<span>Progress</span>
										<span>
											{quest.progress}/{quest.totalTasks}
										</span>
									</div>
									<div className="mt-1 w-full bg-gray-200 rounded-full h-2">
										<div
											...
											style={{
												width: `${(quest.progress / quest.totalTasks) * 100}%`,
											}}
										/>
									</div>
								</div>
								<div className="mt-4 flex items-center justify-between text-xs text-gray-500">
									<span>
										Due{" "}
										{formatDistanceToNow(new Date(quest.deadline), {
											addSuffix: true,
										})}
									</span>
									<span
