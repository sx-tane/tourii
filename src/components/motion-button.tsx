...
const MotionButton: React.FC<MotionButtonProps> = ({
    ...
    return (
        <motion.button
            className={`border-[1px] ${borderColor} rounded-full font-medium overflow-hidden whitespace-nowrap ${textSize} ${className} md:block`}
            initial={{
            ...
            onTouchStart={() => {
                // On mobile/tablet, directly trigger click instead of hover
                if (onClick) onClick();
            }}
            onClick={(e) => {
        ...
        >
            ...
            >
                ...
                <motion.span
                    className="absolute font-light tracking-widest italic md:text-lg text-sm hidden md:block"
                    animate={{
