...
const RouteDiscovery: React.FC<RouteDiscoveryProps> = memo(({
	...
	const hasResults = aiRoutes?.generatedRoutes?.length || filteredExistingRoutes.length;
		...
		return (
			<div className="flex flex-col items-center justify-center py-20">
				...
				<p className="text-charcoal/70 text-center max-w-md">
					Our AI is analyzing {selectedInterests.length > 0 ? `${selectedInterests.join(', ')}` : 'your preferences'} 
					{region ? ` in ${region}` : ''} to create personalized recommendations.
				</p>
			</div>
		);
	}
		...
		return (
			<div className="flex flex-col items-center justify-center py-20 text-center">
				<div className="w-16 h-16 bg-charcoal/10 rounded-full flex items-center justify-center mb-4">
					<svg className="w-8 h-8 text-charcoal/50" fill="currentColor" viewBox="0 0 20 20">
				...
				<p className="text-charcoal/70 mb-6 max-w-md">
					...
					{region ? ` in ${region}` : ''}. Try different interests or explore other regions.
				</p>
			</div>
		);
	}
		...
		<div className="w-full max-w-7xl mx-auto">
			...
			>
				...
				<p className="text-lg text-charcoal/80 mb-4">
					Based on your interests: {selectedInterests.map(interest => `#${interest}`).join(', ')}
				</p>
				...
				{aiRoutes?.summary && (
					<div className="flex justify-center gap-6 text-sm text-charcoal/60">
						...
						<span>{aiRoutes.summary.routesGenerated} AI routes generated</span>
						<span>{filteredExistingRoutes.length} existing routes</span>
						{aiRoutes.summary.processingTimeMs && (
							<span>Generated in {(aiRoutes.summary.processingTimeMs / 1000).toFixed(1)}s</span>
						)}
					</div>
				)}
			</motion.div>
					...
					>
						<div className="flex items-center gap-3 mb-6">
							...
							<span className="bg-red text-white px-3 py-1 rounded-full text-sm font-medium">
								{aiRoutes.generatedRoutes.length} route{aiRoutes.generatedRoutes.length !== 1 ? 's' : ''}
							</span>
						</div>
					...
					>
						<div className="flex items-center gap-3 mb-6">
							...
							<span className="bg-charcoal text-white px-3 py-1 rounded-full text-sm font-medium">
								{filteredExistingRoutes.length} route{filteredExistingRoutes.length !== 1 ? 's' : ''}
							</span>
						</div>
...
const RouteCard: React.FC<RouteCardProps> = memo(({ route, isAiGenerated, index }) => {
	const hasImage = route.regionBackgroundMedia || (route.touristSpots && route.touristSpots[0]);
	const imageUrl = route.regionBackgroundMedia || 
		(route.touristSpots && route.touristSpots[0] && `/image/model-route/1/placeholder.jpg`);
			...
			<div className="p-6">
				...
				{(route.recommendations || route.recommendation) && (
					<div className="flex flex-wrap gap-2 mb-4">
						{(route.recommendations || route.recommendation)?.slice(0, 3).map((tag: string) => (
							<span
								key={tag}
				...
				{/* Action Button */}
				<Link
					href={`/v2/model-route/${route.modelRouteId}`}
