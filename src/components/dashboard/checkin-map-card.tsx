...
const CheckinMapCard: React.FC<CheckinMapCardProps> = ({
	...
	// Fetch checkins data for summary only if no mock data is provided
	...
	const actualCheckins = mockCheckins || checkinsData?.checkins || [];
	const totalCheckins = mockCheckins
		? mockCheckins.length
		: checkinsData?.total || 0;

	// Calculate stats using actual checkins
	const storyCheckins = actualCheckins.filter((c) => c.type === "story").length;
	const questCheckins = actualCheckins.filter((c) => c.type === "quest").length;
	const routeCheckins = actualCheckins.filter((c) => c.type === "route").length;

	// Get recent checkin
	const recentCheckin = actualCheckins[0];
