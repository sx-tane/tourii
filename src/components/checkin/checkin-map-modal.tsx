...
const CheckinMapModal: React.FC<CheckinMapModalProps> = ({
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'story' | 'quest'>('all');
  const [selectedCheckin, setSelectedCheckin] = useState<CheckinResponseDto | null>(null);
  const [isLocationModalOpen, setIsLocationModalOpen] = useState(false);

  // Fetch checkins data
  const { data: checkinsData, isLoading, isError } = useCheckins({
    userId,
    type: activeFilter,
    limit: 100,
  });
  ...
  const filteredCheckins = useMemo(() => {
    ...
    if (activeFilter === 'all') {
      return checkinsData.checkins;
    }
    
    return checkinsData.checkins.filter(checkin => checkin.type === activeFilter);
  }, [checkinsData, activeFilter]);
            ...
            {isOpen && (
              ...
              >
                ...
                <div className="flex-1 relative">
                  ...
                  {isError && (
                    ...
                    >
                      <div className="text-center">
                        ...
                        <p className="text-gray-600">Failed to load check-ins</p>
                        <button
                          type="button"
                  ...
                  {!isLoading && !isError && filteredCheckins.length === 0 && (
                    <motion.div
                      className="absolute inset-0 flex items-center justify-center bg-gray-50 z-10"
                      initial={{ opacity: 0, y: 20 }}
                      animate={{ opacity: 1, y: 0 }}
                    >
                      <div className="text-center">
                        <div className="text-4xl mb-4">📍</div>
                        <p className="text-gray-600 mb-2">No check-ins found</p>
                ...
                {/* Stats Footer */}
                {!isLoading && !isError && checkinsData && (
                  <motion.div
                  ...
                  >
                    <div className="flex items-center justify-between text-sm text-gray-600">
                      <span>
                        Showing {filteredCheckins.length} of {checkinsData.total} check-ins
                      </span>
