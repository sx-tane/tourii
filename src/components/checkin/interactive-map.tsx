	...
	useEffect(() => {
		if (typeof window !== "undefined") {
			import("leaflet")
				.then((L) => {
					try {
						// Fix default markers - use CDN URLs as fallback for Storybook
						...
						// Try to use local images, fall back to CDN
						try {
						...
						logger.error("Failed to configure Leaflet icons", {
							error: iconError,
						});
					}
...
const InteractiveMap: React.FC<InteractiveMapProps> = ({
	...
	}, [checkins, onMarkerClick, L]);

	// Add timeout for loading fallback
	useEffect(() => {
		const timeout = setTimeout(() => {
			if (isLoading && !L) {
				logger.warn("Map loading timed out, showing fallback");
				setIsLoading(false);
			}
	...
	}, [L, leafletLoading, initializeMap]);
